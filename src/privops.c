...
#if 1 //modify 20160907 //由vsftpd的配置项去限制权限，此处不做限制
  p_sess->permissions = (Global_WRITE|Global_READ);
#endif
...
handle_local_login(struct vsf_session* p_sess,
                   ...
                   const struct mystr* p_pass_str)
{
#if 0 //modify 20160907
  if (!vsf_sysdep_check_auth(p_user_str, p_pass_str, &p_sess->remote_ip_str))
  {
    return kVSFLoginFail;
  }
#else
  if(cos_handle_login(p_user_str, p_pass_str, &p_sess->permissions) != 0)
  {
    return kVSFLoginFail;
  }
#endif
