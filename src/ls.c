...
static const char* time_get_date(const time_t mtime, int use_localtime, long curr_time);
static void build_dir_line(struct mystr* p_str,
                           const struct mystr* p_filename_str,
                           //const struct vsf_sysutil_statbuf* p_stat,
                           const struct file_info* p_stat,
                           long curr_time);
void get_user_root_dir(struct mystr* p_root_dir, struct vsf_session* p_sess);
                         ...
                         int is_stat)
{
...
#else
  (void) p_dir;
  static struct mystr full_filename = INIT_MYSTR;
  str_copy(&full_filename, &normalised_base_dir_str);
  str_append_str(&full_filename, p_filter_str);
  static struct file_info info = {0, INIT_MYSTR, 0, 0, 0};
  // STAT
  if (is_stat)
  {
	  if (cos_stat_file(p_sess, 10, &full_filename, &info) == 0 ||
		  cos_stat_dir(p_sess, 10, &full_filename, &info) == 0)
	  {
		  if (is_verbose)
		  {
			  build_dir_line(&dirline_str, &full_filename, &info, curr_time);
		  }
		  else
		  {
			  str_copy(&dirline_str, &full_filename);
			  str_append_text(&dirline_str, "\r\n");
		  }
		  str_list_add(p_list, &dirline_str, &full_filename);
	  }
  }
  // LIST
  else
  {
      ...
      if (!str_isempty(p_filter_str))
      {
          ...
          {
              // 不是正则
              if (str_all_normal_char(p_filter_str))
                  goto free_and_out;
              filter_func = vsf_filename_passes_filter;
          }
      }
      //not file
      if (str_list_get_length(p_list) == 0)
      {
          void *result = cos_get_list(p_sess, 30, &normalised_base_dir_str, filter_func, p_filter_str);
          while (cos_list_next(result, &info) == 0)
          {
              /* Calculate the full path (relative to CWD) for lstat() and
               * output purposes
               */
              static struct mystr s_next_filename_str;
              static struct mystr s_next_path_and_filename_str;
              str_copy(&s_next_filename_str, &info.file_name);
              str_copy(&s_next_path_and_filename_str, &normalised_base_dir_str);
              str_append_str(&s_next_path_and_filename_str, &s_next_filename_str);

              if (is_verbose)
              {
                  build_dir_line(&dirline_str, &s_next_filename_str, &info, curr_time);
              }
              else
              {
                  /* Just emit the filenames - note, we prepend the directory for NLST
               ...
               */
              {
                  ...
                  else
                  {
                      ...
                      snprintf(intbuf, sizeof(intbuf), "%030ld", (long) info.mtime);
                      str_alloc_text(&s_temp_str, intbuf);
                      p_sort_str = &s_temp_str;
                      p_sort_subdir_str = &s_temp_str;
                  }
                  str_list_add(p_list, &dirline_str, p_sort_str);
                  if (p_subdir_list != 0 && info.type == TYPE_DIR)
                  {
                      str_list_add(p_subdir_list, &s_next_filename_str, p_sort_subdir_str);
                  }
              }
          }
          cos_free_list(result);
          str_list_sort(p_list, r_option);
          if (p_subdir_list != 0)
          {
              str_list_sort(p_subdir_list, r_option);
...
build_dir_line(struct mystr* p_str, const struct mystr* p_filename_str,
               const struct file_info* p_stat, long curr_time)
{
...
#else
  filesize_t size = p_stat->file_size;
  /* Permissions */
  //char perms[11] = {'-','r','w','x','r','w','x','r','-','-',0};
  char perms[11] = {'-','-','-','-','-','-','-','-','-','-',0};
  perms[0] = (p_stat->type==1 ? '-' : 'd');
  str_alloc_text(p_str, perms);
  str_append_char(p_str, ' ');
  /* Hard link count */
  str_alloc_ulong(&s_tmp_str, 1);
#endif
  ...
  str_append_char(p_str, ' ');
  /* Date stamp */
  str_append_text(p_str, time_get_date(p_stat->mtime, tunable_use_localtime, curr_time));
  str_append_char(p_str, ' ');
  /* Filename */
...
const char*
time_get_date(const time_t mtime, int use_localtime, long curr_time)
{
...
void
get_user_root_dir(struct mystr* p_root_dir, struct vsf_session* p_sess)
{
    (void)p_sess;
    str_alloc_text(p_root_dir, tunable_user_home_dir);
	if (str_isempty(p_root_dir) ) {
		str_append_text(p_root_dir, "./");
	}
	
	if (str_get_char_at(p_root_dir, str_getlen(p_root_dir) - 1) != '/')
    {
      str_append_char(p_root_dir, '/');
    }
	
	str_append_text(p_root_dir, tunable_cos_appid);
	str_append_char(p_root_dir, '/');
	str_append_text(p_root_dir, tunable_cos_bucket);
}
