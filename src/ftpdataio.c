...
static struct vsf_transfer_ret do_file_send_rwloop(
  struct vsf_session* p_sess, int file_fd, int is_ascii, filesize_t offset, filesize_t file_size);
static struct vsf_transfer_ret do_file_recv(
  struct vsf_session* p_sess, int file_fd, int is_ascii, long long have_recv_len);
...
static unsigned int get_cos_download_chunksize();
                      ...
                      int is_verbose)
{
  ...
  vsf_ls_populate_dir_list(p_sess, &dir_list, p_subdir_list, p_dir, p_base_dir_str,
                           p_option_str, p_filter_str, is_verbose, is_control);
  if (p_subdir_list)
  {
    int retval;
...
vsf_ftpdataio_transfer_file(struct vsf_session* p_sess, int remote_fd,
                            int file_fd, int is_recv, int is_ascii, long long have_recv_len, long long file_size)
{
  if (!is_recv)
  {
    ...
    {
      return do_file_send_rwloop(p_sess, file_fd, is_ascii, have_recv_len, file_size);
    }
    else
    {
  ...
  {
    return do_file_recv(p_sess, file_fd, is_ascii, have_recv_len);
  }
}

static struct vsf_transfer_ret
do_file_send_rwloop(struct vsf_session* p_sess, int file_fd, int is_ascii, filesize_t offset, filesize_t file_size)
{
#if 0
  (void) offset;
  (void) file_size;
  ...
  (void) file_fd;
  static char* p_asciibuf;
  static char* p_downloadbuf;
  static unsigned int s_download_bufsize = 0;
  struct vsf_transfer_ret ret_struct = { 0, 0 };
  unsigned int chunk_size = get_chunk_size();
  char* p_readbuf;
  char* p_writefrom_buf;
  int prev_cr = 0;
  int dlbuf_curpos = 0, dlbuf_len = 0;
  if (p_downloadbuf == 0)
  {
    s_download_bufsize = get_cos_download_chunksize();
    vsf_secbuf_alloc(&p_downloadbuf, s_download_bufsize);
  }
  if (is_ascii && p_asciibuf == 0)
  {
    /* NOTE!! * 2 factor because we can double the data by doing our ASCII
     * linefeed mangling
     */
  ...
  while (1)
  {
    /* fill downloadbuf from cos */
    if (dlbuf_curpos >= dlbuf_len)
    {
      if (offset >= file_size)
      {
        dlbuf_len = 0;
      ...
      {
        dlbuf_len = cos_download_file(p_sess, 10, &p_sess->ftp_arg_str,
                p_downloadbuf, s_download_bufsize, offset, file_size);
        offset += dlbuf_len;
      }
      dlbuf_curpos = 0;
    }

    /* fill readbuf from downloadbuf */
    unsigned int num_to_write;
    int retval = 0;
    if (dlbuf_len <= 0)
    {
      retval = dlbuf_len;
    } else if (dlbuf_curpos < dlbuf_len)
    {
      unsigned int left = dlbuf_len - dlbuf_curpos;

      p_readbuf = p_downloadbuf + dlbuf_curpos;
      retval = (chunk_size<left ? chunk_size : left);

      dlbuf_curpos += retval;
    }

    /* check retval and write to client */
    if (vsf_sysutil_retval_is_error(retval))
    {
      ret_struct.retval = -1;
      return ret_struct;
...
static struct vsf_transfer_ret
do_file_recv(struct vsf_session* p_sess, int file_fd, int is_ascii, long long have_recv_len)
{
...
#if 1 //modify 20150921 for max file size
    static long long max_upload_file_size = -1;
    if (max_upload_file_size < 0)
        max_upload_file_size = tunable_max_upload_file_size * (long long)1024;

	if((max_upload_file_size > 0
                && ret_struct.transferred+have_recv_len) > max_upload_file_size) {
		ret_struct.retval = -3;
      	return ret_struct;
	}
#endif
...
static unsigned int
get_cos_download_chunksize()
{
  #define VSFTP_COS_DOWNLOAD_BUFSIZE (64*1024*1024)
  unsigned int ret = VSFTP_COS_DOWNLOAD_BUFSIZE;
  if (tunable_cos_download_chunk_size < VSFTP_COS_DOWNLOAD_BUFSIZE &&
      tunable_cos_download_chunk_size > 0)
  {
    ret = tunable_cos_download_chunk_size;
    if (ret < 65536)
    {
      ret = 65536;
    }
