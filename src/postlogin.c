...
static void prepend_session_path_to_filename(struct vsf_session* p_sess, struct mystr* p_str);
...
extern int str_mkdirs(const struct mystr* p_dir_str, const unsigned int mode);

void
process_post_login(struct vsf_session* p_sess)
{
#if 1 //modify 20160905
  cos_env_init();
#endif
  ...
  while(1)
  {
...
#if 1 //modify 20150922
	cos_conv_gbk_2_utf(p_sess, &p_sess->ftp_arg_str);
#endif
    if (tunable_setproctitle_enable)
    {
             ...
             str_equal_text(&p_sess->ftp_cmd_str, "EPSV"))
    {
      handle_pasv(p_sess, 1);
             ...
             (p_sess->permissions & Global_READ) &&
             ...
             str_equal_text(&p_sess->ftp_cmd_str, "PORT"))
    {
      handle_port(p_sess);
             ...
             (p_sess->permissions & Global_WRITE) &&
    ...
    {
#if 0 //modify 20150922
      handle_rnfr(p_sess);
#else
      vsf_cmdio_write(p_sess, FTP_COMMANDNOTIMPL, "RNFR not implemented.");
#endif
    }
    else if (tunable_write_enable &&
             (tunable_anon_other_write_enable || !p_sess->is_anonymous) &&
             ...
             str_equal_text(&p_sess->ftp_cmd_str, "RNTO"))
    {
      ...
      handle_rnto(p_sess);
#else
      vsf_cmdio_write(p_sess, FTP_COMMANDNOTIMPL, "RNTO not implemented.");
#endif
    }
    else if (tunable_dirlist_enable &&
             (p_sess->permissions & Global_READ) &&
    ...
    else if ((p_sess->permissions & Global_READ) &&
             str_equal_text(&p_sess->ftp_cmd_str, "SIZE"))
    {
      handle_size(p_sess);
    }
             ...
             str_equal_text(&p_sess->ftp_cmd_str, "SITE"))
    {
      ...
      handle_site(p_sess);
#else
      vsf_cmdio_write(p_sess, FTP_COMMANDNOTIMPL, "SITE not implemented.");
#endif
    }
    ...
    {
#if 0 //modify 20151014
      handle_appe(p_sess);
#else
      vsf_cmdio_write(p_sess, FTP_COMMANDNOTIMPL, "APPE not implemented.");
#endif
    }
    else if (str_equal_text(&p_sess->ftp_cmd_str, "MDTM"))
    {
#if 0 //modify 20151019
      handle_mdtm(p_sess);
#else
      vsf_cmdio_write(p_sess, FTP_COMMANDNOTIMPL, "MDTM not implemented.");
#endif
    }
    else if (tunable_port_enable &&
             ...
             str_equal_text(&p_sess->ftp_cmd_str, "STOU"))
    {
      ...
      handle_stou(p_sess);
#else
      vsf_cmdio_write(p_sess, FTP_COMMANDNOTIMPL, "STOU not implemented.");
#endif
    }
...
#if 0 //modify 20150919
  str_getcwd(&s_cwd_buf_mangle_str);
#else
  str_session_getcwd(p_sess, &s_cwd_buf_mangle_str);
  if (str_isempty(&s_cwd_buf_mangle_str))
	  str_append_char(&s_cwd_buf_mangle_str, '/');
#endif
...
#else
  if (!vsf_access_session_check_file(p_sess, &p_sess->ftp_arg_str))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
  retval = cos_isdir(p_sess, 10, &p_sess->ftp_arg_str);
  if (retval == 0)
  {
	  struct mystr disk_path = INIT_MYSTR;
	  str_copy(&disk_path, &p_sess->ftp_arg_str);
	  prepend_session_path_to_filename(p_sess, &disk_path);
	  retval = str_chdir(&disk_path);
	  if (retval != 0)
	  {
		  retval = str_mkdirs(&disk_path, 0777);
		  if (retval == 0)
			  retval = str_chdir(&disk_path);
	  }
	  str_free(&disk_path);
...
handle_retr(struct vsf_session* p_sess, int is_http)
{
...
#else
  (void) s_p_statbuf;
  prepend_session_path_to_filename(p_sess, &p_sess->log_str);
  if (!vsf_access_session_check_file(p_sess, &p_sess->ftp_arg_str))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  ...
  int retval = cos_stat_file(p_sess, 10, &p_sess->ftp_arg_str, &info);
  if (retval != 0)
  {
      vsf_cmdio_write(p_sess, FTP_FILEFAIL, "Failed to open file.");
      return;
...
#else
  str_append_filesize_t(&s_mark_str, info.file_size);
#endif
  ...
  trans_ret = vsf_ftpdataio_transfer_file(p_sess, remote_fd,
                                          opened_file, 0, is_ascii, 0, 0);
#else
  trans_ret = vsf_ftpdataio_transfer_file(p_sess, remote_fd,
                                          opened_file, 0, is_ascii, offset, info.file_size);
#endif
  if (!is_http &&
...
handle_dir_common(struct vsf_session* p_sess, int full_details, int stat_cmd)
{
  ...
  if (!str_isempty(&s_filter_str))
  {
    ...
    if (p_dir != 0)
#else // file use stat_cmd
    if (!vsf_access_session_check_file(p_sess, &s_filter_str))
    {
      vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
      return;
    }

    if (!stat_cmd && cos_isdir(p_sess, 10, &s_filter_str) == 0)
#endif
    {
      /* Listing a directory! */
...
handle_upload_common(struct vsf_session* p_sess, int is_append, int is_unique)
{
...
#else
  static struct mystr s_local_filename;
  str_copy(&s_local_filename, &p_sess->ftp_arg_str);
  prepend_session_path_to_filename(p_sess, &s_local_filename);
  p_filename = &s_local_filename;
#endif
  if (is_unique)
  {
	  // TODO
...
#else
  prepend_session_path_to_filename(p_sess, &p_sess->log_str);
  if (!vsf_access_session_check_file(p_sess, p_filename))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
  ...
  if (tunable_lock_upload_files)
  {
#if 0 //modify 20151015
    vsf_sysutil_lock_file_write(new_file_fd);
#else
	if (vsf_sysutil_lock_file_write_addi(new_file_fd) != 0)
	{
		vsf_cmdio_write(p_sess, FTP_BADFILEBUSY, "File is locked, Try later.");
		// 由阻塞改成非阻塞后,为防止文件被误删,设置为成功.
		success = 1;
		goto port_pasv_cleanup_out;
	}
#endif
  ...
  if (!is_append && offset != 0)
  {
#if 1 //modify 20151015
	filesize_t file_len = vsf_sysutil_statbuf_get_size(s_p_statbuf);
	if (file_len < offset)
	{
		vsf_cmdio_write(p_sess, FTP_BADSENDFILE, "Offset is out of bounds. Try again");
		cos_unlink(p_sess, 10, p_filename);
		goto port_pasv_cleanup_out;
	}
#endif
    /* XXX - warning, allows seek past end of file! Check for seek > size? */
    vsf_sysutil_lseek_to(new_file_fd, offset);
	have_recv_len = offset;
  }
  else if (is_append)
  {
    vsf_sysutil_lseek_end(new_file_fd);
	have_recv_len = vsf_sysutil_statbuf_get_size(s_p_statbuf);
  }
  if (is_unique)
  {
    struct mystr resp_str = INIT_MYSTR;
#if 0 //modify 20160905
    str_alloc_text(&resp_str, "FILE: ");
    str_append_str(&resp_str, p_filename);
#else
    struct mystr user_root_dir = INIT_MYSTR;
	get_user_root_dir(&user_root_dir, p_sess);
	str_copy(&resp_str, p_filename);
	str_replace_text(&resp_str, str_getbuf(&user_root_dir), "FILE: ");
	str_free(&user_root_dir);
#endif
  ...
  {
    trans_ret = vsf_ftpdataio_transfer_file(p_sess, remote_fd,
                                            new_file_fd, 1, 1, have_recv_len, 0);
  }
  else
  {
    trans_ret = vsf_ftpdataio_transfer_file(p_sess, remote_fd,
                                            new_file_fd, 1, 0, have_recv_len, 0);
  }
...
#else
  else if (trans_ret.retval == -3)
  {
	char err_msg[64] = {0};
	snprintf(err_msg, sizeof(err_msg), "The max allow file size is: %u MB", tunable_max_upload_file_size/1024);
    vsf_cmdio_write(p_sess, FTP_BADSENDFILE, err_msg);
	str_unlink(p_filename);
  }
  else if (trans_ret.retval == 0)
  {
#if 1 //modify 20151019
    port_cleanup(p_sess);
#endif
	struct mystr debug = INIT_MYSTR;
	str_copy(&debug, p_filename);
	str_append_text(&debug, "  Receive success.");
	vsf_log_line(p_sess, kVSFLogEntryDebug, &debug);
	str_free(&debug);

	if(0 != cos_upload_file(p_sess, 360, p_filename, &p_sess->ftp_arg_str, tunable_cos_upload_slice_size)) {
		vsf_cmdio_write(p_sess, FTP_BADSENDFILE, "Failure transfer file to cos.");
	} else {
		success = 1;
		vsf_log_do_log(p_sess, 1);
	    vsf_cmdio_write(p_sess, FTP_TRANSFEROK, "Transfer complete.");
		vsf_sysutil_unlink(str_getbuf(p_filename));
	}
  }
#endif
  check_abor(p_sess);
...
handle_mkd(struct vsf_session* p_sess)
{
  ...
  retval = cos_mkdir(p_sess, 10, &p_sess->ftp_arg_str);
  if (retval == 0 || retval == -178)
  {
	  static struct mystr s_mkd_path;
	  str_copy(&s_mkd_path, &p_sess->ftp_arg_str);
	  prepend_session_path_to_filename(p_sess, &s_mkd_path);
	  str_mkdir(&s_mkd_path, 0777);
  }
  if (retval != 0)
  {
	  if (retval == -178)
...
handle_rmd(struct vsf_session* p_sess)
{
  ...
  retval = cos_dir_isempty(p_sess, 10, &p_sess->ftp_arg_str);
  if (retval == 1)
  {
    vsf_cmdio_write(p_sess, FTP_FILEFAIL,
                    "Remove directory operation failed (not empty).");
    return;
  }
  else if (retval != 0)
  {
  ...
  retval = cos_rmdir(p_sess, 10, &p_sess->ftp_arg_str);
  if (retval == 1)
  {
    vsf_cmdio_write(p_sess, FTP_FILEFAIL, "Remove directory operation failed (not exist).");
    return;
  }
  if (retval == 0)
  {
...
handle_dele(struct vsf_session* p_sess)
{
  ...
  retval = cos_unlink(p_sess, 10, &p_sess->ftp_arg_str);
  if (retval == 1)
  {
    vsf_cmdio_write(p_sess, FTP_FILEFAIL, "Delete operation failed (not exist).");
    return;
  }
#endif
  if (retval != 0)
...
#if 1 //modify 20150921
static void
prepend_session_path_to_filename(struct vsf_session *p_sess, struct mystr* p_str)
{
  static struct mystr s_tmp_str;
  /* Only prepend current working directory if the incoming filename is
   * relative
  ...
  {
	get_user_root_dir(&s_tmp_str, p_sess);
  }
  str_append_str(&s_tmp_str, p_str);
  str_copy(p_str, &s_tmp_str);
}

void real_path(struct mystr* p_path)
{
	prepend_path_to_filename(p_path);
	do {
		unsigned int org_len = str_getlen(p_path);
		str_replace_text(p_path, "/./", "/");
		if (str_getlen(p_path) == org_len)
			break;
	} while(1);
	struct mystr rhs = INIT_MYSTR;

	do {
		unsigned int org_len = str_getlen(p_path);
		str_split_text(p_path, &rhs, "/../");
		if (str_getlen(p_path) == org_len)
			break;
		struct mystr temp_str = INIT_MYSTR;
		str_split_char_reverse(p_path, &temp_str, '/');
		str_free(&temp_str);
		if (!str_isempty(&rhs))
		{
			str_append_char(p_path, '/');
			str_append_str(p_path, &rhs);
		}
	} while(1);

	str_free(&rhs);
}
void session_real_path(struct vsf_session* p_sess, struct mystr* p_path)
{
	prepend_session_path_to_filename(p_sess, p_path);
	do {
		unsigned int org_len = str_getlen(p_path);
		str_replace_text(p_path, "/./", "/");
		if (str_getlen(p_path) == org_len)
...
#else
	  static struct file_info info;
	  retval = cos_stat_file(p_sess, 10, &p_sess->ftp_arg_str, &info);
	  if (retval != 0)
		  vsf_cmdio_write(p_sess, FTP_FILEFAIL, "Could not get file size.");
	  else
	  {
		  static struct mystr s_size_res_str;
		  str_alloc_filesize_t(&s_size_res_str, info.file_size);
...
handle_site(struct vsf_session* p_sess)
{
  ...
  else if ((p_sess->permissions & Global_WRITE) &&
          str_equal_text(&p_sess->ftp_arg_str, "UMASK"))
  {
