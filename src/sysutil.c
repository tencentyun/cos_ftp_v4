...
const char*
vsf_sysutil_int_to_str(int the_int)
{
  static char int_buf[32];
  (void) snprintf(int_buf, sizeof(int_buf), "%d", the_int);
  return int_buf;
}
...
int
vsf_sysutil_exist_access(const char* p_filename)
{
  int retval = access(p_filename, F_OK);
  return (retval == 0);
}
...
int
vsf_sysutil_lock_file_write_addi(int fd)
{
  int retval;
  int saved_errno;
  do
...
int
vsf_sysutil_unlock_file_addi(int fd)
{
