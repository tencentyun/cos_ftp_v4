...
#if 1 //modify 20160901
const char* tunable_cos_appid;
const char* tunable_cos_secretid;
const char* tunable_cos_secretkey;
const char* tunable_cos_bucket;
const char* tunable_cos_download_domain;
const char* tunable_user_home_dir;
const char* tunable_cos_region;

const char* tunable_login_users;

unsigned int tunable_cos_conn_timeout_ms;
unsigned int tunable_cos_req_timeout_ms;
unsigned int tunable_cos_sign_expire_s;
unsigned int tunable_cos_upload_slice_size;
int tunable_cos_take_sha;

unsigned int tunable_max_upload_file_size;
unsigned int tunable_list_max_count;
unsigned int tunable_cos_download_chunk_size;
#endif
...
#if 1 //modify 20160901
  install_str_setting("", &tunable_cos_appid);
  install_str_setting("", &tunable_cos_secretid);
  install_str_setting("", &tunable_cos_secretkey);
  install_str_setting("", &tunable_cos_bucket);
  install_str_setting("gz", &tunable_cos_region);
  install_str_setting("cdn", &tunable_cos_download_domain);
  install_str_setting("/tempdir", &tunable_user_home_dir);
  install_str_setting(0, &tunable_login_users);

  tunable_cos_conn_timeout_ms = 10*1000;
  tunable_cos_req_timeout_ms = 60*1000;
  tunable_cos_sign_expire_s = 5*60;
  tunable_cos_take_sha = 0;
  tunable_cos_upload_slice_size = 1024*1024; //1MB

  tunable_max_upload_file_size = 0;
  tunable_list_max_count = 0;
