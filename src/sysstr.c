...
/* start with '/', not end with '/' */
void
str_session_getcwd(struct vsf_session* p_sess, struct mystr* p_str)
{
	str_getcwd(p_str);
	static struct mystr user_root_dir;
	get_user_root_dir(&user_root_dir, p_sess);
	str_replace_text(p_str, str_getbuf(&user_root_dir), "");
}
