...
void
str_append_int(struct mystr* p_str, int the_int)
{
  str_append_text(p_str, vsf_sysutil_int_to_str(the_int));
}
...
int
str_all_normal_char(const struct mystr* p_str)
{
  unsigned int i;
  for (i=0; i < p_str->len; i++)
  {
    if (! (vsf_sysutil_isalnum(p_str->p_buf[i])
                || p_str->p_buf[i] == '-' || p_str->p_buf[i] == '_' || p_str->p_buf[i] == '.'))
    {
      return 0;
    }
  }
